...
- `WorkspaceSession` 作为正式后端会话类型进入 `store`，不再只靠前端页面语义区分
- `waiting_approval` / `waiting_input` 仍占用 active budget，直到对应 turn 进入终态
- 远程 workspace 准备阶段先不加新 RPC，先用固定的 `mkdir -p` 跑通
- Planner / Worker 先使用固定 internal preset，不先暴露为可配置 profile
...
在调度器对象之外，还需要给现有 `store.SessionState` 增加正式的后端会话元数据，否则无法把 `WorkspaceSession / PlannerSession / WorkerSession` 与普通单机会话区分开来。

建议新增：

```go
type SessionMeta struct {
    Kind                 string // single_host | workspace | planner | worker
    Visible              bool
    MissionID            string
    WorkspaceSessionID   string
    WorkerHostID         string
    RuntimePreset        string // single_host_default | workspace_front | planner_internal | worker_internal
}
```

说明：

- `WorkspaceSession` 必须成为正式 `kind=workspace` 的后端 session
- `PlannerSession` / `WorkerSession` 默认 `visible=false`
- 普通聊天历史、主机历史会话默认过滤 `visible=false` 的内部 session
- 审批镜像、输入镜像、详情跳转都依赖 `MissionID / WorkspaceSessionID / WorkerHostID`
...
2. 在第一次启动该 Worker 前，由调度器通过现有 host exec transport 执行固定命令 `mkdir -p <RemotePath>`
3. 这条 bootstrap 命令属于 orchestrator 内部基础设施准备，不作为 Worker 面向用户的任务 transcript，也不生成 Worker 审批镜像卡片；但需要写审计
4. 后续 WorkerSession 默认把这个路径作为 `cwd`
...
- 动态工具不能再只按“当前 session 是否选中了 remote host”来分发
- `handleDynamicToolCall` 需要先解析 `SessionMeta.Kind`
- `kind=planner` 时进入 planner tool registry
- `kind=worker` 时进入现有 remote host tool registry
- `kind=workspace` / `kind=single_host` 不暴露 `orchestrator_dispatch_tasks`

也就是说，MVP 需要的是 **按 session kind 分域的动态工具路由**，而不是简单在现有 remote tools switch 里加一个新 case
...
### 10.0 Planner / Worker 固定 preset

由于当前 profile 体系只正式支持 `main-agent` 和 `host-agent-default`，第一阶段不要把 Planner / Worker 做成新的公开 profile 类型。

建议在 `internal/orchestrator/preset.go` 内定义两个固定 preset：
...
- `cwd=<default-workspace>/missions/<missionID>/planner`
- 允许 MCP / skills / `orchestrator_dispatch_tasks`
- 不允许 remote host execute_* 动态工具
...
- `approvalPolicy=untrusted`
- `sandboxMode=workspace-write`
- `cwd=<remote workspace path>`
- 只允许当前 host 的 remote execute_* 动态工具
- 不允许 planner 专用派发工具
- 不允许直接在抽屉或详情页中作为用户可交互会话暴露

后续如果要开放配置，再把 preset 提升成正式 profile 配置项。
...
- 前台工作台会话为 `kind=workspace`、`visible=true`
- PlannerSession 为 `kind=planner`、`visible=false`
...
- Worker 处于 `waiting_approval` / `waiting_input` 时，仍然继续占用 active budget；不要在这里释放 slot
...
- 先给 `internal/codex.Client` 增加 `PendingCount()` 之类的可观测接口
- 在没有这个接口之前，MVP 只能声称支持 `thread/start` / `turn/start` 限速，不能声称真正支持 `PendingRequestBudget`
...
4. 某个 Worker 的 turn 进入终态（`completed / failed / cancelled / aborted`）后，再释放预算
...
- Worker thread 空闲超过一定 TTL 时自动清理本地绑定
- mission 结束后统一清理所有内部 session 的 thread 绑定
- thread 丢失时允许按现有逻辑自动重建

这里的“回收”在第一阶段应明确定义为：

- `store.ClearThread(sessionID)`
- 让调度器停止继续复用旧 thread

而不是要求当前 Codex 集成必须提供显式 `thread/delete` RPC。
...
同样的路由原则也适用于 `request_user_input`：

- 如果 PlannerSession / WorkerSession 触发 `request_user_input`
- WorkspaceSession 展示镜像 `ChoiceCard`
- 用户作答后，还是路由回原始 internal session

这样可以避免：

- 内部 session 自己出现在前台等待用户输入
- `approval` 和 `choice` 两套镜像逻辑分叉
...
- `handleChatMessage` 需要按 `SessionMeta.Kind` 分流
- `kind=single_host` 继续走现有路径
- `kind=workspace` 走 mission / planner 路径
- `kind=planner` / `kind=worker` 拒绝前台直接发消息
...
- `handleChatStop` 也要按 `SessionMeta.Kind` 分流：工作台 stop 取消 mission，单机会话 stop 保持原逻辑
...
- `orchestrator_dispatch_tasks`
- session-kind aware dynamic tool registry
...
- WorkspaceSession -> WorkerSession 的 approval 路由解析
- WorkspaceSession -> PlannerSession / WorkerSession 的 choice 路由解析
...
- `SessionMeta`
- 创建 session 时指定 kind / visible / preset 的能力
- `SessionSummaries()` 过滤 invisible internal sessions
- `HostSessions()` 默认过滤 invisible internal sessions
- 为 orchestrator 提供按 `MissionID / WorkspaceSessionID / WorkerHostID` 反查 session 的能力

### 17.5 `internal/orchestrator/*`
...
- session-kind aware dynamic tool routing
- `orchestrator_dispatch_tasks`
- `Manager.Dispatch`
...
- active exec cancel
- workspace-level approval / choice 路由
